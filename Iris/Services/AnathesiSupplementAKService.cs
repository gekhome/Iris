...
{
    public class AnathesiSupplementAKService : IAnathesiSupplementAKService, IDisposable
    {
        private readonly IrisDBEntities entities;

        public AnathesiSupplementAKService(IrisDBEntities entities)
        {
            this.entities = entities;
        }

        public IEnumerable<AnathesiSupplementAKViewModel> Read(int schoolyearId, int schoolId)
        {
            var data = (from d in entities.ΑΝΑΘΕΣΕΙΣ_ΣΥΜΠΛΗΡΩΜΑΤΙΚΕΣ_ΑΚ
                        orderby d.ΑΝΑΘΕΣΗ_ΚΩΔ descending
                        where d.ΣΧΟΛΙΚΟ_ΕΤΟΣ == schoolyearId && d.ΣΧΟΛΗ == schoolId
                        select new AnathesiSupplementAKViewModel
                        {
        ...
        public IEnumerable<AnathesiSupplementAKViewModel> Read(ApofasiParameters ap)
        {
            List<AnathesiSupplementAKViewModel> data = new List<AnathesiSupplementAKViewModel>();

            if (ap.apofasiId > 0)
            {
                data = (from d in entities.ΑΝΑΘΕΣΕΙΣ_ΣΥΜΠΛΗΡΩΜΑΤΙΚΕΣ_ΑΚ
        ...
        public void Create(AnathesiSupplementAKViewModel data, int schoolyearId, int schoolId)
        {
            ...
            entities.ΑΝΑΘΕΣΕΙΣ_ΣΥΜΠΛΗΡΩΜΑΤΙΚΕΣ_ΑΚ.Add(entity);
            entities.SaveChanges();

            data.ΑΝΑΘΕΣΗ_ΚΩΔ = entity.ΑΝΑΘΕΣΗ_ΚΩΔ;
        }

        public void Update(AnathesiSupplementAKViewModel data, int schoolyearId, int schoolId)
        {
            ΑΝΑΘΕΣΕΙΣ_ΣΥΜΠΛΗΡΩΜΑΤΙΚΕΣ_ΑΚ entity = entities.ΑΝΑΘΕΣΕΙΣ_ΣΥΜΠΛΗΡΩΜΑΤΙΚΕΣ_ΑΚ.Find(data.ΑΝΑΘΕΣΗ_ΚΩΔ);
        ...
        public void Update(AnathesiSupplementAKViewModel data, ApofasiParameters ap)
        {
            ΑΝΑΘΕΣΕΙΣ_ΣΥΜΠΛΗΡΩΜΑΤΙΚΕΣ_ΑΚ entity = entities.ΑΝΑΘΕΣΕΙΣ_ΣΥΜΠΛΗΡΩΜΑΤΙΚΕΣ_ΑΚ.Find(data.ΑΝΑΘΕΣΗ_ΚΩΔ);
        ...
        public void Destroy(AnathesiSupplementAKViewModel data)
        {
            ΑΝΑΘΕΣΕΙΣ_ΣΥΜΠΛΗΡΩΜΑΤΙΚΕΣ_ΑΚ entity = entities.ΑΝΑΘΕΣΕΙΣ_ΣΥΜΠΛΗΡΩΜΑΤΙΚΕΣ_ΑΚ.Find(data.ΑΝΑΘΕΣΗ_ΚΩΔ);
            ...
            if (entity != null)
            {
                ...
                entities.ΑΝΑΘΕΣΕΙΣ_ΣΥΜΠΛΗΡΩΜΑΤΙΚΕΣ_ΑΚ.Remove(entity);
                entities.SaveChanges();
            }
        }

        public AnathesiSupplementAKViewModel Refresh(int entityId)
        {
            return entities.ΑΝΑΘΕΣΕΙΣ_ΣΥΜΠΛΗΡΩΜΑΤΙΚΕΣ_ΑΚ.Select(d => new AnathesiSupplementAKViewModel
            {
                ΑΝΑΘΕΣΗ_ΚΩΔ = d.ΑΝΑΘΕΣΗ_ΚΩΔ,
                ΑΦΜ = d.ΑΦΜ,
                ΕΠΩΝΥΜΟ = d.ΕΠΩΝΥΜΟ,
