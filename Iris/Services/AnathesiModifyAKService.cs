...
{
    public class AnathesiModifyAKService : IAnathesiModifyAKService, IDisposable
    {
        private readonly IrisDBEntities entities;

        public AnathesiModifyAKService(IrisDBEntities entities)
        {
            this.entities = entities;
        }

        public IEnumerable<AnathesiModifyAKViewModel> Read(int schoolyearId, int schoolId)
        {
            var data = (from d in entities.ΑΝΑΘΕΣΕΙΣ_ΤΡΟΠΟΠΟΙΗΤΙΚΕΣ_ΑΚ
                        orderby d.ΑΝΑΘΕΣΗ_ΚΩΔ descending
                        where d.ΣΧΟΛΙΚΟ_ΕΤΟΣ == schoolyearId && d.ΣΧΟΛΗ == schoolId
                        select new AnathesiModifyAKViewModel
                        {
        ...
        public IEnumerable<AnathesiModifyAKViewModel> Read(ApofasiParameters ap)
        {
            List<AnathesiModifyAKViewModel> data = new List<AnathesiModifyAKViewModel>();

            if (ap.apofasiId > 0)
            {
                data = (from d in entities.ΑΝΑΘΕΣΕΙΣ_ΤΡΟΠΟΠΟΙΗΤΙΚΕΣ_ΑΚ
        ...
        public void Create(AnathesiModifyAKViewModel data, int schoolyearId, int schoolId)
        {
            ...
            entity.ΑΠΟΦΑΣΗ_ΕΙΔΟΣ = "ΤΡΟΠΟΠΟΙΗΤΙΚΗ-ΑΚ";
            ...
            entities.ΑΝΑΘΕΣΕΙΣ_ΤΡΟΠΟΠΟΙΗΤΙΚΕΣ_ΑΚ.Add(entity);
            entities.SaveChanges();

            data.ΑΝΑΘΕΣΗ_ΚΩΔ = entity.ΑΝΑΘΕΣΗ_ΚΩΔ;
        }

        public void Update(AnathesiModifyAKViewModel data, int schoolyearId, int schoolId)
        {
            ΑΝΑΘΕΣΕΙΣ_ΤΡΟΠΟΠΟΙΗΤΙΚΕΣ_ΑΚ entity = entities.ΑΝΑΘΕΣΕΙΣ_ΤΡΟΠΟΠΟΙΗΤΙΚΕΣ_ΑΚ.Find(data.ΑΝΑΘΕΣΗ_ΚΩΔ);
        ...
        public void Update(AnathesiModifyAKViewModel data, ApofasiParameters ap)
        {
            ΑΝΑΘΕΣΕΙΣ_ΤΡΟΠΟΠΟΙΗΤΙΚΕΣ_ΑΚ entity = entities.ΑΝΑΘΕΣΕΙΣ_ΤΡΟΠΟΠΟΙΗΤΙΚΕΣ_ΑΚ.Find(data.ΑΝΑΘΕΣΗ_ΚΩΔ);
        ...
        public void Destroy(AnathesiModifyAKViewModel data)
        {
            ΑΝΑΘΕΣΕΙΣ_ΤΡΟΠΟΠΟΙΗΤΙΚΕΣ_ΑΚ entity = entities.ΑΝΑΘΕΣΕΙΣ_ΤΡΟΠΟΠΟΙΗΤΙΚΕΣ_ΑΚ.Find(data.ΑΝΑΘΕΣΗ_ΚΩΔ);
            ...
            if (entity != null)
            {
                ...
                entities.ΑΝΑΘΕΣΕΙΣ_ΤΡΟΠΟΠΟΙΗΤΙΚΕΣ_ΑΚ.Remove(entity);
                entities.SaveChanges();
            }
        }

        public AnathesiModifyAKViewModel Refresh(int entityId)
        {
            return entities.ΑΝΑΘΕΣΕΙΣ_ΤΡΟΠΟΠΟΙΗΤΙΚΕΣ_ΑΚ.Select(d => new AnathesiModifyAKViewModel
            {
                ΑΝΑΘΕΣΗ_ΚΩΔ = d.ΑΝΑΘΕΣΗ_ΚΩΔ,
                ΑΦΜ = d.ΑΦΜ,
                ΕΠΩΝΥΜΟ = d.ΕΠΩΝΥΜΟ,
